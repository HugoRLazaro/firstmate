...
Under the extension model (Pi, pi-signed, and omp) a live identity-matched watcher is the ordinary healthy state, but a fresh beacon without one is also healthy while direct relay evidence proves the chain is still handing off, because `.pi/extensions/fm-primary-pi-watch.ts` and `.omp/extensions/fm-primary-omp-watch.ts` tear the watcher down on every actionable wake and spawn the replacement themselves.
`fm_extension_relay_healthy` in `bin/fm-wake-lib.sh` owns that evidence: the session origin must be alive (the lock names a live process), and either the newest record in `state/.watch-cycle-exits.log` names a live `started:`/`attached:` successor (`fm_watch_cycle_successor_alive`, identity-matched when the record carries an identity) or the Pi arm declaration `state/.pi-watch-extension-arm` names this lock-owning session's active generation plus a live arm child or a pending retry (`fm_pi_extension_arm_pending`).
A recorded `successor=none` with no declared pending attempt and a recorded successor whose process is gone are not relay evidence on their own, so each stays down unless the Pi arm declaration or the omp marker proof still carries the hand-off; a lost session origin stays down for both families.
The omp primary publishes no arm declaration, so it instead keeps the marker-ownership hand-off proof as relay evidence with any lifecycle record whenever the lock is genuinely unheld; that is the omp restoration window while the markered, live session respawns the watcher.
Under Pi, only when no usable lifecycle record exists at all does the decision fall back to that same shared proof `fm_extension_owns_supervision`, which accepts either the Pi pair (`fm_pi_extension_owns_supervision`) or the omp pair (`fm_omp_extension_owns_supervision`): both primary extensions of one family must be recorded in their state markers at their current on-disk builds by the process named in `state/.lock`, and that process must still be alive; omp never inherits the Pi arm-declaration tolerance because its proof is keyed on its own two files and markers.
The marker-ownership proof still requires a genuinely unheld lock - the lock directory or its symlinked owner directory is absent, or the existing lock records no pid at all - because a lock with a recorded pid that fails the strict watcher health check is not a hand-off.
...
Without any of that evidence an unheld lock alarms exactly as it did before, so an unloaded, version-drifted, or exited Pi or omp session is loud immediately, and a cycle the extension never restores is loud once the beacon passes grace.
...
`tests/fm-guard-stale-banner.test.sh` covers the pull-guard predicate, including the persistent-model fresh-leftover-beacon negative control; the auto-arm model's healthy fresh-beacon-without-a-watcher case, session-and-recovery-bound long-turn rewake tolerance, independently broken tolerance signals, open-claim negative control, stale-beacon alarm, and isolation from other models; and the extension model's live-watcher path, ownership-qualified fresh hand-off, ledger- and declaration-qualified relay windows, `successor=none` and dead-successor alarms, lost-session-origin alarm, held-lock failures, independently broken evidence signals, stale-beacon alarm, queued-wake warning, and Pi and pi-signed harness routing.
