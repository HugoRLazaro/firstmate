...
A recorded `successor=none` with no declared pending attempt stays down, a recorded successor whose process is gone stays down, and a lost session origin stays down.
Only when no usable lifecycle record exists at all does the decision fall back to the marker-ownership hand-off proof `fm_extension_owns_supervision`, which accepts either the Pi pair (`fm_pi_extension_owns_supervision`) or the omp pair (`fm_omp_extension_owns_supervision`): both primary extensions of one family must be recorded in their state markers at their current on-disk builds by the process named in `state/.lock`, and that process must still be alive; omp never inherits the Pi tolerance because its proof is keyed on its own two files and markers.
That fallback still requires a genuinely unheld lock - the lock directory or its symlinked owner directory is absent, or the existing lock records no pid at all - because a lock with a recorded pid that fails the strict watcher health check is not a hand-off.
