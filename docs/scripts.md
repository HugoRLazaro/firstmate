...
| `fm-edicion.sh`          | The firstmate half of the `/edicion` route: open a project clone's marking surface, read the marks it leaves in `data/edicion/`, and apply them as one task or into a live task |
...
| `fm-tasks-axi-lib.sh`    | Shared backlog-backend and `tasks-axi` binary selector plus compatibility probe      |
...
| `fm-backpass-lane.sh`    | Run the backpass memory-review worker over an isolated byte-for-byte memory copy, publishing its proposal and never applying it |
...
| `fm-teardown.sh`         | Fail-closed teardown: return landed ship worktrees (pruning the returned copy's Treehouse pool) or close a released-worktree record, require completed scout deliverables, retire secondmate homes |
