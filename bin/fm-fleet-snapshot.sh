...
# Large payloads travel by file, never by argv: the backlog can exceed the
# system's single-argument limit (E2BIG) and must stay readable by jq.
...
if [ "$OUTPUT_MODE" = contribution-input ]; then
  ...
  jq -n --slurpfile backlog "$BACKLOG_JSON_FILE" --slurpfile tasks "$CONTRIBUTION_TASKS_JSON_FILE" \
    ...
    || { echo "fm-fleet-snapshot: contribution input projection failed" >&2; exit 1; }
  exit 0
fi
